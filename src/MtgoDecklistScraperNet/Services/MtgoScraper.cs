...
public class MtgoScraper
{
	...
	public async Task RunAsync(int? year = null, int? month = null)
	{
		if (year.HasValue)
		{
		...
		try
		{
			var indexHtml = await _client.FetchIndexAsync(year, month);
		...
		for (var i = 0; i < links.Count; i++)
		{
			...
			try
			{
				var eventHtml = await _client.FetchEventPageAsync(link);
				...
				await _saver.SaveEventAsync(link, mtgoEvent);
			}
